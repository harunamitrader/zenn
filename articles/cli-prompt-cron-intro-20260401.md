---
title: "AIチャットだけで導入から定期実行設定まで進められる cli-prompt-cron"
emoji: "⏰"
type: "tech"
...
Gemini CLI や Codex、Claude Code を使っていると、
...
これは、AI CLI 向けに **定期実行** を足すためのツールなのですが、個人的にいちばん面白いのは、**導入からジョブ作成までを AI チャットでかなり進められる** ところです。
...
- `safe / edit / plan / full` の4段階で権限を統一して扱える
...
特に気に入っているのは、**CLI ごとの権限差を `safe / edit / plan / full` に寄せている**ところです。

内部ではツールごとに別のオプションへ変換していますが、使う側は4つのラベルだけ覚えておけばよくなります。
...
ここでのポイントは、**ツールを入れる作業そのものも AI に寄せられる**ことです。
...
*保存される JSON もかなり読みやすいです。実コマンドを丸ごと覚えなくていいのが大きいです。*

ここで良いのは、JSON に実行コマンドを直書きせず、
...
逆に、単発の対話しかせず、定期実行も運用も不要なら少し重いかもしれません。
...
「AI を使うツール」ではなく、**AI にツール運用も手伝わせる**方向へ一歩進めたい人には、かなり相性がいいです。
